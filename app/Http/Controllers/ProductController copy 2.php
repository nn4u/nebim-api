...
class ProductController extends Controller
{
    ...
    private function formatVariantss(array $variants): string
    {
        return implode(',', array_map(function ($variant) {
            ...
            return "{barcode: \"{$variant['barcode']}\", price: \"{$variant['price']}\", optionValues: [{$optionValues}]}";
        }, $variants));
    }


    private function formatVariants1(array $variants): string
{
    return implode(',', array_map(function ($variant) {
        $optionValues = implode(',', array_map(function ($ov) {
            return "{name: \"{$ov['name']}\", optionName: \"{$ov['optionName']}\"}";
        }, $variant['optionValues']));
        return "{barcode: \"{$variant['barcode']}\", price: \"{$variant['price']}\", sku: \"{$variant['sku']}\", inventoryManagement: null, inventoryPolicy: NOT_MANAGED, optionValues: [{$optionValues}]}";
    }, $variants));
}
...
private function formatVariants(array $variants): string
{
    ...
    private function formatUpdateVariantss(array $variants): string
    {
        return implode(',', array_map(function ($variant) {
            return "{id: \"{$variant['id']}\", barcode: \"{$variant['barcode']}\", price: \"{$variant['price']}\"}";
        }, $variants));
    }
